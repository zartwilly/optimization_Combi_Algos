...
#
###############################################################################
def calculate_OF_value_N_solutions(All_N_for_i, X0):
    """
    ...
    Returns
    -------
    OF_Values_for_N_i, OF_Values_all_N: array of shape (1, len(X0)+1), array of shape (N, len(X0)+1).
    ...
    # Calculating OF for the i-th solution in N
    for solution in All_N_for_i:
        
        OF_value = fitness_value(initial_solution=solution)
        
        solution = solution[np.newaxis]
        
        OF_Values_for_N_i = np.column_stack((OF_value, solution))               # Stack the OF value to the deertment vector
        
        OF_Values_all_N = np.vstack((OF_Values_all_N, OF_Values_for_N_i))
        
        N_Count = N_Count+1
        
    return OF_Values_for_N_i, OF_Values_all_N
...
#
###############################################################################
def get_neighborhood(initial_solution):
    """
    ...
    """
    List_of_N = list(itr.combinations(initial_solution, 2))                     # From X0, it shows how many combinations of 2's can it get; 8 choose 2
    
    All_N_for_i = np.empty((0,len(initial_solution)))
...
def check_solution_in_Tabu_List(OF_Values_all_N_Ordered, Tabu_List, 
                                Length_of_Tabu_List, Save_Solutions_Here):
    """
    ...
    return Save_Solutions_Here, Current_Sol, Tabu_List
...
#
###############################################################################
def make_diversifisation(Iterations, X0, Current_Sol):
    """
...
def Tabu_Search_Algorithm(X0):
    ...
    for i in range(RUNS):
        ...
        print()
        print(f"--> This is the { Iterations}-th Iteration <--")
    
    
        # To create all surrounding neighborhood
        
        All_N_for_i = None
        All_N_for_i = get_neighborhood(initial_solution=X0)
        
        OF_Values_for_N_i = np.empty((0,len(X0)+1))                             # +1 to add the OF values
        OF_Values_all_N = np.empty((0,len(X0)+1))                               # +1 to add the OF values
    
        OF_Values_for_N_i, OF_Values_all_N = calculate_OF_value_N_solutions(
                                                All_N_for_i, X0)
        
        # Ordered OF of neighborhood, sorted by OF value
        OF_Values_all_N_Ordered = np.array(sorted(OF_Values_all_N,
                                                  key=lambda x: x[0]))
        
        Save_Solutions_Here, Current_Sol, Tabu_List \
            = check_solution_in_Tabu_List(OF_Values_all_N_Ordered, 
                                          Tabu_List, 
                                          Length_of_Tabu_List, 
                                          Save_Solutions_Here)
        
        Current_Sol = make_diversifisation(Iterations, X0, Current_Sol)
        X0 = Current_Sol[1:]
        print(f" Current_Sol = {Current_Sol}")
...
def compute_OF_solution_i_of_N_NEW(All_N_for_i, OF_Values_for_N_i, 
                                   OF_Values_all_N):
...
def check_solution_in_Tabu_List_NEW(OF_Values_all_N_Ordered, Tabu_List, 
                                    Length_of_Tabu_List):
...
def make_diversifisation_NEW(Current_Sol, X0, Iterations, Length_of_Tabu_List):
...
def Tabu_Search_Algorithm_NEW(X0):
    """
    Tabu search Algorithm 
    ...
    for i in range(RUNS):
        ...
        All_N_for_i = None
        All_N_for_i = get_neighborhood_NEW(X0)
        ...
        OF_Values_all_N = compute_OF_solution_i_of_N_NEW(All_N_for_i, 
                                                         OF_Values_for_N_i, 
                                                         OF_Values_all_N)
        ...
        # Check if solution is already in Tabu list, if yes, choose the next one
        Current_Sol, Tabu_List = check_solution_in_Tabu_List_NEW(
        ...
        # In order to "kick-start" the search when stuck in a local optimum, for diversification
        Current_Sol, Length_of_Tabu_List = make_diversifisation_NEW(
                                                Current_Sol, X0, 
...
if __name__ == "__main__":
    ...
    Tabu_Search_Algorithm_NEW(X0=initial_solution)
    pass